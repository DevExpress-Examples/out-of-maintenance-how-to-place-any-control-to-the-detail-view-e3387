...
Namespace CustomGrid
	Public Class CustomPainter
		...
		Public Overrides Sub Draw(ByVal e As ViewDrawArgs)
            Dim aView As CustomView = (TryCast(View, CustomView))
            If aView.GridControl.IsDesignMode Then
				ShowTitle(e)
				Return
			End If
            aView.ShowControl(e)
		...
		Protected Overridable Sub ShowTitle(ByVal e As ViewDrawArgs)
            Dim aView As CustomView = (TryCast(View, CustomView))
            Dim text As String = String.Format("{0}: {1}", aView.LevelName, aView.Name)
            e.Cache.FillRectangle(Color.White, e.Bounds)
			e.Cache.DrawRectangle(New Pen(Color.Gray, 1), e.Bounds)
            Dim font As New Font(aView.GridControl.Font.FontFamily, 18)
